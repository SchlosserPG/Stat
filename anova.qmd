...
::: {.callout-note}
This lesson combines the ANOVA swirl exercises and introduces correlation using the Boston dataset. Parts 1–3 cover one-way and two-way ANOVA with post-hoc tests. Part 4 covers correlation and scatterplots. All datasets are built into base R — no packages or CSV files needed.
...
### Part 1: One-Way ANOVA — ChickWeight
...
**Load and summarize ChickWeight:**
...
::: {.callout-tip}
`ChickWeight` contains chick weights over time across four diet groups. We will test whether diet affects weight.
:::

**Build the ANOVA model and view results:**

```{webr-r}
#| label: chick-anova
# H0: all diet group means are equal
# HA: at least one mean differs
model <- aov(weight ~ Diet, ChickWeight)
anova(model)
```

::: {.callout-note}
The p-value is well below 0.001 — we reject H₀ and conclude that at least one diet group has a significantly different mean weight. The omnibus test tells us *something* differs but not *which* groups. That requires a post-hoc test.
:::

**Run Tukey's HSD to identify which groups differ:**
...
::: {.callout-tip}
Each row compares two groups. A p-value below 0.05 indicates those two groups are significantly different. The `diff` score tells you the direction — a positive diff means the second group listed has a higher mean than the first. Groups 2−1, 3−2, and 4−3 are not significantly different; all other pairwise comparisons are.
:::

---

### Part 2: One-Way ANOVA — PlantGrowth

**Load and summarize PlantGrowth:**
...
```{webr-r}
#| label: plant-anova
# H0: ctrl, trt1, and trt2 all have equal mean weight
# HA: at least one mean differs
model2 <- aov(weight ~ group, PlantGrowth)
anova(model2)
```
...
::: {.callout-note}
Only `trt2 − trt1` is significant (p < 0.05). The control group does not differ significantly from either treatment. The negative diff for `trt1 − trt2` tells you that treatment 1 has a *lower* mean weight than treatment 2.
:::

---

### Part 3: Two-Way ANOVA — mtcars
...
**Prepare the data — convert gear and cyl to factors:**

```{webr-r}
#| label: mtcars-prep
mtcars$gear <- as.factor(mtcars$gear)
mtcars$cyl  <- as.factor(mtcars$cyl)
summary(mtcars[, c("mpg", "gear", "cyl")])
```

**Build the two-way ANOVA model:**

```{webr-r}
#| label: twoway-anova
# H0 (gear): all gear groups have equal mean mpg
# H0 (cyl):  all cyl groups have equal mean mpg
model3 <- aov(mpg ~ gear + cyl, mtcars)
anova(model3)
```

::: {.callout-note}
Two F-statistics and two p-values — one for each factor. Both gear and cyl are significant here, meaning each independently explains variation in mpg. A two-way ANOVA with `+` tests main effects only. To test whether the *interaction* between gear and cyl also matters, you would use `gear * cyl` instead.
:::

**Run Tukey's HSD for both factors:**
...
::: {.callout-tip}
Read the gear and cyl sections separately. For gear: groups 5−4 are not significantly different; 4−3 and 5−3 are. For cyl: groups 8−6 are not significantly different; 6−4 and 8−4 are. The diff scores confirm that gear 5 has higher mpg than gear 3, and cyl 8 has lower mpg than cyl 4.
:::

---

### Part 4: Correlation — Boston Dataset
...
::: {.callout-tip}
Key variables: `medv` = median house value (in $1,000s), `lstat` = % households with low socioeconomic status, `rm` = average rooms per house, `age` = proportion of homes built before 1940.
:::
...
ggplot(Boston, aes(x = lstat, y = medv)) +
  ...
  labs(title = "Low Economic Status vs. Median House Value",
       x = "% Low Economic Status (lstat)",
       y = "Median House Value ($000s)")
```
...
::: {.callout-note}
The correlation is approximately −0.74 — a strong negative linear relationship. As the percentage of low economic status households increases, median house value decreases. The p-value is far below 0.05, confirming the correlation is highly significant. The 95% confidence interval does not include zero, which corroborates the significant p-value.
:::

**Now try the age variable:**
...
ggplot(Boston, aes(x = age, y = medv)) +
  ...
  stat_smooth(method = "lm", color = "red") +
  labs(title = "Age of Housing Stock vs. Median House Value",
       x = "Proportion Built Before 1940 (age)",
       y = "Median House Value ($000s)")
```

```{webr-r}
#| label: cortest-age
cor.test(Boston$age, Boston$medv)
```

::: {.callout-note}
The correlation with `age` is also significant but much weaker (approximately −0.38). A higher proportion of older homes is associated with lower median values, but `age` explains far less of the variability in `medv` than `lstat` does. Always compare the *magnitude* of the correlation alongside the p-value — statistical significance does not equal practical importance.
:::
...
<div id="ps512-anova-quiz" style="font-family: Georgia, serif; max-width: 720px; margin: 2rem auto; background: #fafaf8; border: 1px solid #ddd; border-radius: 8px; padding: 2rem;">
  ...
  <div class="aquiz-q" id="aq3" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">3. In the ChickWeight ANOVA, the correct R code to build the model is:</p>
    <label class="aquiz-option"><input type="radio" name="aq3" value="a"> <code>aov(Diet ~ weight, ChickWeight)</code></label><br>
    <label class="aquiz-option"><input type="radio" name="aq3" value="b"> <code>aov(weight ~ Diet, ChickWeight)</code></label><br>
    <label class="aquiz-option"><input type="radio" name="aq3" value="c"> <code>lm(weight ~ Diet, ChickWeight)</code></label><br>
    <label class="aquiz-option"><input type="radio" name="aq3" value="d"> <code>t.test(weight ~ Diet, ChickWeight)</code></label>
  ...
  <div class="aquiz-q" id="aq5" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">5. In the PlantGrowth Tukey output, only trt2 − trt1 is significant. This means:</p>
    <label class="aquiz-option"><input type="radio" name="aq5" value="a"> All three groups differ from each other.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq5" value="b"> Only treatment 2 and treatment 1 are significantly different; the control does not differ from either treatment.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq5" value="c"> The control group has the highest mean weight.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq5" value="d"> The overall ANOVA was not significant.</label>
    <div class="aquiz-feedback" id="afb5"></div>
  </div>

  <!-- Q6 -->
  <div class="aquiz-q" id="aq6" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">6. Why must <code>gear</code> and <code>cyl</code> be converted to factors before running a two-way ANOVA on the mtcars dataset?</p>
    <label class="aquiz-option"><input type="radio" name="aq6" value="a"> R cannot read numeric variables in aov().</label><br>
    <label class="aquiz-option"><input type="radio" name="aq6" value="b"> They are stored as numbers but represent categories — without coercion, R treats them as continuous predictors instead of grouping variables.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq6" value="c"> Factors run faster in R than numeric variables.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq6" value="d"> TukeyHSD() only works on factor variables.</label>
    <div class="aquiz-feedback" id="afb6"></div>
  </div>

  <!-- Q7 -->
  <div class="aquiz-q" id="aq7" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">7. A two-way ANOVA with <code>aov(mpg ~ gear + cyl, mtcars)</code> produces how many F-statistics?</p>
    <label class="aquiz-option"><input type="radio" name="aq7" value="a"> One — for the combined effect of gear and cyl.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq7" value="b"> Two — one for gear and one for cyl.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq7" value="c"> Three — one for each level of gear.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq7" value="d"> Four — one for each level of cyl.</label>
    <div class="aquiz-feedback" id="afb7"></div>
  </div>

  <!-- Q8 -->
  <div class="aquiz-q" id="aq8" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">8. The correlation between lstat and medv in the Boston dataset is approximately −0.74. This means:</p>
    <label class="aquiz-option"><input type="radio" name="aq8" value="a"> A weak positive relationship — as lstat increases, medv slightly increases.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq8" value="b"> A strong negative relationship — as lstat increases, medv tends to decrease.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq8" value="c"> No linear relationship between the two variables.</label><br>
    <label class="aquiz-option"><input type="radio" name="aq8" value="d"> lstat causes medv to decrease.</label>
...
const aHints = {
  ...
  aq3:  "aov() uses Y ~ X format: the outcome (weight) on the left, the grouping variable (Diet) on the right, followed by the dataset.",
  aq4:  "A negative diff means the first group listed has a lower mean than the second. For example, diff for '3-1' being positive means group 3 > group 1.",
  aq5:  "Only the trt2 vs trt1 p-value is below 0.05. The control (ctrl) does not differ significantly from either treatment — only the two treatments differ from each other.",
  aq6:  "Numeric variables stored as integers (3, 4, 5 gears; 4, 6, 8 cylinders) would be treated as continuous if not coerced. as.factor() tells R to treat them as discrete categories.",
  aq7:  "A two-way ANOVA with + produces one F-statistic per grouping variable — two total. The * operator would add a third for the interaction term.",
  aq8:  "r = −0.74 is a strong negative correlation. Direction is negative (as one goes up, the other goes down) and strength is substantial. Note: correlation is not causation.",
...
This lesson covered group comparison with ANOVA and linear association with correlation:
...
**What comes next:** The Regression lesson picks up directly from correlation. Where correlation describes the strength and direction of the relationship between two continuous variables, regression uses that relationship to build a predictive model — estimating a slope and intercept so that we can forecast values of the outcome variable. The goodness-of-fit measures introduced there (R², adjusted R², RSE) build on the sum-of-squares logic covered in this lesson. Regression is the last major topic we have in BUAD231 and is the foundation for advanced analytics.
