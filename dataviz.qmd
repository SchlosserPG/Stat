...
1. Common Graphs for a Single _Categorical_ Variable
2. Common Graphs for a Single _Continuous_ Variable
...
* ggplot() works in layers, so you will routinely see the _+_ symbol to kick off a new layer with added functionality. 
* Using the ggplot, we always include the aes command first inside the ggplot() command. The aes() command is a _quoting function_ that describes the variables being used. From there, it depends on the plot. 
...
MarketShare <- ggplot(data_frame, aes(x = Category, y = Percentage, fill = Category)) +
     ...
     labs(title = "How do you expect R's market share to change?",
          ...
          y = "Percentage (%)") +
     theme_minimal()+
     geom_text(aes(label = Percentage), vjust = -0.5, size = 4); MarketShare
```
...
* AUQ300 needs to be a factor variable with 1 equaling a _Yes_ and 2 equaling a _No_. We can use recode_factor to accomplish 2 things at once with the mutate function. 
...
* Add an axis labels under _labs(x = ..., y=...)_.
...
legalize.bar <- gss.2016.cleaned %>% 
  drop_na(grass) %>% 
  ggplot(aes(x = grass)) + geom_bar(fill=c("red","purple")) +
...
* _Probability density_ is not very useful for interpreting what is happening at any given value of the variable on the x-axis, but it is useful in computing the percentage of values that are within a range along the x-axis. 
* The _area under the curve_ in a density plot could be interpreted as the probability of a single observation or a range of observations.
...
```{r fig.alt = "Grouped Bar Graph Generated by R"}
mtcars <- mtcars %>% mutate(vs=as.factor(vs)) %>% mutate(gear=as.factor(gear))
...
mtcars %>%
     ...
     geom_boxplot(aes(fill = vs), show.legend = FALSE) +
...
* This plot has a clear positive trend, meaning that as one has more years of education, we see higher income. And similarly, as we see higher income, we also see more years of education. This means that a scatter can help characterize the relationship, and does _not_ state that one variable is causing another to occur. 
...
```{r, fig.alt = "Scatterplot With stat_smooth Generated from ggplot2 by R"}
ggplot(Edu, aes(x=Education, y=Income)) +
    geom_point(color="#183028", shape=18, size=4) +
    labs(y= "Income", x = "Education") +
    stat_smooth(color="#789F90")
```
...
```{r, fig.alt = "Scatterplot With stat_smooth lm method Generated from ggplot2 by R"}
ggplot(Edu, aes(x=Education, y=Income)) +
    geom_point() +
    labs(y= "Income", x = "Education") +
    stat_smooth(method="lm", color="#789F90")
...
```{r, fig.alt = "Scatterplot examples in R"}
ggplot(mtcars, aes(x=qsec, y=mpg))+geom_point() + stat_smooth(method="lm", color="#789F90")
```
...
ggplot(mtcars, aes(x=vs, y=mpg))+geom_point() +
...
library(MASS)
data("UScrime")
ggplot(UScrime, aes(x=Po1, y=GDP)) + geom_point() + stat_smooth(method="lm")
ggplot(UScrime, aes(x=Ed, y=y)) + geom_point() + stat_smooth(method="lm")
ggplot(UScrime, aes(x=Po1, y=y)) + geom_point() + stat_smooth(method="lm")
ggplot(UScrime, aes(x=GDP, y=y)) + geom_point() + stat_smooth(method="lm")
```
...
* Specifically, AUQ060 refers to "Hear a whisper from across a quiet room?" AUQ070 refers to "Hear normal voice across a quiet room?" and AUQ080 refers to "Hear a shout from across a quiet room?" While AUQ300 refers to "Ever used firearms for any reason? AUQ310 refers to "How many total rounds ever fired?" and AUQ320 refers to "Wear hearing protection when shooting?" I got these references in the nhanes_auditory_2012_codebook.html file. 
...
* That is a few of the charts. See if you can determine any thing from them and try to run the other combinations. You will see that it is easier to see with less categories when you have multiple variables like this. 
