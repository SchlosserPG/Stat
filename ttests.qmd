...
### Step 2. Compute the Test Statistic and calculate the p-value
...
The `sleep` dataset requires reshaping from long to wide format before the paired test can run. Two approaches produce the same result — `reshape()` from base R, or `filter()` + `full_join()` from tidyverse. Both create a dataset where each patient's Drug 1 and Drug 2 measurements are in separate columns on the same row, which is what `paired = TRUE` requires. The interactive lesson below walks through this step by step.
...
::: {.callout-note}
This lesson covers the one-sample, independent samples, and dependent (paired) samples t-test using built-in R datasets. All calculations run entirely in the browser — no packages or data files needed.
...
**Does engine type (vs) or transmission (am) affect mpg?**

```{webr-r}
#| label: indep-mtcars
# Engine type: H0: mu_vs0 = mu_vs1
t.test(mtcars$mpg ~ mtcars$vs)

# Transmission: H0: mu_am0 = mu_am1
t.test(mtcars$mpg ~ mtcars$am)

# Directional: do automatics (am=0) get less mpg than manuals (am=1)?
...
**Does cold treatment affect CO2 uptake? Two-tailed and directional:**
...
# Directional: nonchilled (group 1) > chilled (group 2)?
...
**Load, reshape, and run the paired t-test:**
