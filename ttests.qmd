...
-   Using tidyverse, we could get the same effect using the filter, select, and rename functions which are a part of dplyr. In the code below, we use filter to divide by group and then after selecting and renaming the columns we need, we join back together using a full_join.
...
## Using AI
* Use the following prompt on a generative AI, like chatGPT to learn more about t.test's and confidence intervals. 
