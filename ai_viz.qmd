...
> "I have a dataset called gig.csv with a numeric variable `Wage` (hourly rate in dollars) and a factor variable `Industry` (Automotive, Construction, Tech). Using R, please: (1) calculate the mean, median, standard deviation, and IQR of Wage for the full dataset; (2) calculate the same statistics broken out by Industry using `tapply()` or `group_by()` + `summarise()`; (3) run `skew()` from the semTools package to check whether Wage is normally distributed; and (4) write a two-sentence interpretation of what the summary statistics tell us about wage variation across industries."
