...
::: {.callout-note}
This lesson combines all four regression swirl exercises: simple linear regression, qualitative predictors, multiple regression, and interaction terms. All datasets are built into base R or already in your `data/` folder — no additional installation needed.
...
### Part 1: Simple Linear Regression — Boston Dataset
...
```{webr-r}
#| label: boston-head-reg
head(Boston)
```

::: {.callout-tip}
Key variables: `medv` = median house value ($000s), `lstat` = % households with low socioeconomic status, `rm` = average rooms per house, `age` = proportion of homes built before 1940.
:::

**Does the percentage of low economic status (lstat) affect median house value (medv)?**

```{webr-r}
#| label: simple-lm-lstat
firstLM <- lm(medv ~ lstat, data = Boston)
summary(firstLM)
```

::: {.callout-note}
Key things to read in the output: the **slope estimate** for lstat (negative — an inverse relationship), the **p-value** on lstat (significant if < 0.05), the **R²** (how much variance in medv is explained), and the **F-statistic p-value** (overall model significance). Here lstat explains about 54% of variance in medv — substantial for a single predictor.
:::
...
::: {.callout-tip}
`confint()` gives a 95% confidence interval around the slope and intercept. We are 95% confident the true slope for lstat falls within that range. Because the entire interval is negative, we are confident the relationship is negative — not a chance result.
:::

**Now try age as a predictor:**

```{webr-r}
#| label: simple-lm-age
secondLM <- lm(medv ~ age, data = Boston)
summary(secondLM)
```
...
::: {.callout-note}
Age is also a significant predictor but far weaker — R² ≈ 0.14 compared to 0.54 for lstat. The slope is also much closer to zero. `age` here represents the *proportion* of owner-occupied homes built before 1940 — a higher proportion of older stock is associated with slightly lower median values, but the relationship is weak and scattered.
:::
...
**Load and explore the Carseats dataset:**
...
**Visualize shelf location vs. sales:**

```{webr-r}
#| label: plot-shelveloc
...
**Regress lstat and age together on medv:**

```{webr-r}
#| label: multiple-reg
multipleReg <- lm(medv ~ lstat + age, data = Boston)
summary(multipleReg)
```

::: {.callout-note}
Compare the slopes here to the simple regressions above. The lstat slope changes from −0.95 to approximately −1.03 when age is added — this is a key feature of multiple regression. Each slope now reflects the predictor's unique contribution *holding the other constant*. Also notice that age, which was negatively related to medv in simple regression, now shows a small positive slope — a sign that the two predictors share variance.
:::

**Run regression with all Boston predictors:**

```{webr-r}
#| label: reg-all
regAll <- lm(medv ~ ., data = Boston)
summary(regAll)
```

**Remove non-significant predictors one at a time — first drop `age`:**

```{webr-r}
#| label: reg-no-age
regAllnoAge <- lm(medv ~ . - age, data = Boston)
summary(regAllnoAge)
```
...
```{webr-r}
#| label: reg-no-age-indus
regAllnoAgeIndus <- lm(medv ~ . - age - indus, data = Boston)
summary(regAllnoAgeIndus)
```

::: {.callout-tip}
The p-value method for variable selection: remove the predictor with the highest non-significant p-value one at a time, rerun the model, and check whether Adjusted R² holds. Here both `age` and `indus` were non-significant and were removed. All remaining predictors are now significant.
:::
...
```r
library(car)
vif(regAllnoAgeIndus)
```

A VIF below 5 is clean. Between 5 and 10 indicates moderate correlation. Above 10 is a problem — that predictor is too highly correlated with another and its coefficient may be unreliable. For the final Boston model, `rad` and `tax` land between 5 and 10, which is acceptable under the threshold of 10 used in this course.
:::
...
**Does the effect of lstat on medv depend on age?**

```{webr-r}
#| label: interact-boston
interact <- lm(medv ~ lstat * age, data = Boston)
summary(interact)
```
...
::: {.callout-note}
The `lstat:age` coefficient is the interaction term. A significant interaction means the effect of `lstat` on `medv` is not the same at all levels of `age` — it changes as `age` changes. A negative interaction slope here means the combined effect of both predictors together is smaller than the sum of their individual effects — the relationship flattens as both variables increase simultaneously.
:::

::: {.callout-important}
**3D visualization — Run in RStudio**

The `scatterplot3d` package cannot install in WebR. To visualize the interaction surface in RStudio:

```r
library(scatterplot3d)
attach(Boston)
scatterplot3d(age, lstat, medv, angle = 40, color = "maroon", pch = 3)
```

Rotate to different angles (try 20, 40, 140) to see the three-dimensional relationship between all three variables.
:::

**Interaction example — Income × Balance predicting Credit Rating:**

```{webr-r}
#| label: interact-credit
interact2 <- lm(Rating ~ Income * Balance, data = Credit)
summary(interact2)
```

::: {.callout-note}
This model has a very high R² (approximately 0.94) — Income, Balance, and their interaction together explain about 94% of variance in Credit Rating. Both direct effects are positive. The interaction is negative (small coefficient around −0.0003), meaning the positive effect each variable has on Rating becomes slightly less strong as the other variable also increases — the relationship flattens at the high end of both predictors.
:::
...
<div id="ps512-reg-quiz" style="font-family: Georgia, serif; max-width: 720px; margin: 2rem auto; background: #fafaf8; border: 1px solid #ddd; border-radius: 8px; padding: 2rem;">
  ...
  <div class="rquiz-q" id="rq1" style="display:block">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">1. In a simple linear regression of medv on lstat, the slope is −0.95. With medv measured in $000s, a 1-unit increase in lstat corresponds to:</p>
    <label class="rquiz-option"><input type="radio" name="rq1" value="a"> An increase of $950 in median house value.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq1" value="b"> A decrease of $950 in median house value.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq1" value="c"> A decrease of $0.95 in median house value.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq1" value="d"> No change — the slope is not significant.</label>
    <div class="rquiz-feedback" id="rfb1"></div>
  </div>

  <!-- Q2 -->
  <div class="rquiz-q" id="rq2" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">2. The R² for the lstat → medv model is approximately 0.54. This means:</p>
    <label class="rquiz-option"><input type="radio" name="rq2" value="a"> lstat and medv have a correlation of 0.54.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq2" value="b"> lstat explains about 54% of the variance in medv.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq2" value="c"> 54% of the observations fall on the regression line.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq2" value="d"> The model predicts medv with 54% accuracy.</label>
  ...
  <div class="rquiz-q" id="rq3" style="display:none">
    ...
    <label class="rquiz-option"><input type="radio" name="rq3" value="a"> A confidence interval for the predicted values of medv.</label><br>
  ...
  <div class="rquiz-q" id="rq6" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">6. In multiple regression, when lstat and age are both included, the lstat slope changes from −0.95 to −1.03. Why?</p>
    ...
    <label class="rquiz-option"><input type="radio" name="rq6" value="c"> Adding age makes lstat more significant.</label><br>
  ...
  <div class="rquiz-q" id="rq7" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">7. To run a regression on all predictors in Boston except age, you would write:</p>
    <label class="rquiz-option"><input type="radio" name="rq7" value="a"> <code>lm(medv ~ all - age, data = Boston)</code></label><br>
    <label class="rquiz-option"><input type="radio" name="rq7" value="b"> <code>lm(medv ~ . - age, data = Boston)</code></label><br>
    <label class="rquiz-option"><input type="radio" name="rq7" value="c"> <code>lm(medv ~ ., data = Boston, exclude = "age")</code></label><br>
    <label class="rquiz-option"><input type="radio" name="rq7" value="d"> <code>lm(medv - age ~ ., data = Boston)</code></label>
  ...
  <div class="rquiz-q" id="rq9" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">9. In <code>lm(medv ~ lstat * age, data = Boston)</code>, what does the <code>*</code> produce?</p>
    <label class="rquiz-option"><input type="radio" name="rq9" value="a"> Only the interaction term lstat:age — not the direct effects.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq9" value="b"> The direct effects of lstat and age plus the interaction term lstat:age.</label><br>
    <label class="rquiz-option"><input type="radio" name="rq9" value="c"> lstat multiplied by age as a single new variable.</label><br>
...
const rHints = {
  rq1:  "The slope −0.95 is in the same units as medv, which is in $000s. So −0.95 × $1,000 = −$950 per 1-unit increase in lstat.",
  rq2:  "R² (coefficient of determination) is the proportion of variance in Y explained by X. 0.54 means 54% of the variability in medv is accounted for by lstat.",
  ...
  rq6:  "In multiple regression, each slope is a partial coefficient — the effect of that predictor holding all others constant. Shared variance between predictors shifts the estimates.",
  rq7:  "The dot (.) includes all predictors. Subtracting a variable with .- removes it from the model. This is how you do backward selection one variable at a time.",
  ...
  rq9:  "The * operator in lm() expands to both main effects and the interaction: lstat + age + lstat:age. To include only the interaction without main effects, use : instead.",
