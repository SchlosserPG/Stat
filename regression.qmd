...
**lstat → medv: fit, summarize, visualize, and get confidence intervals:**
...
::: {.callout-note}
The slope for lstat is negative — an inverse relationship. lstat explains about 54% of variance in medv. `confint()` gives a 95% confidence interval around the slope and intercept — because the entire interval is negative, the relationship is not a chance result.
:::

**age → medv: a weaker predictor for comparison:**
...
::: {.callout-note}
R drops one category as the reference — here `Bad` is alphabetically first. The intercept represents mean sales when `ShelveLoc = Bad`. With k = 3 categories, R creates k − 1 = 2 dummy variables.
:::

**Gender (2 categories) → Credit Limit:**
...
**Two predictors, then all predictors:**
...
**Variable selection — remove non-significant predictors one at a time:**
