...
**What comes next:** The regression model you built here — predicting a continuous outcome from a mix of continuous and categorical predictors — is the foundation for machine learning methods covered in our business anlaytics major or our MSBA program here in the Mason School of Business. Techniques like k-nearest neighbors, decision trees, and ensemble models all share the same goal of minimizing prediction error, and many of the diagnostics and model-evaluation concepts introduced here (residuals, overfitting, the bias-variance tradeoff) carry forward directly.
