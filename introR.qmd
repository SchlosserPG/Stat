...
::: callout-note
This lesson combines the two swirl exercises on creating and loading data in R. You will build vectors, matrices, and data frames from scratch, use `rnorm()` and `set.seed()`, and explore the Auto dataset — all in the browser. Work through each part in order, then take the scored quiz.
...
**Generate random normal data and control reproducibility with `set.seed()`:**

```{webr-r}
#| label: rnorm-basic
x <- rnorm(20)             # mean 0, sd 1 by default
y <- rnorm(20, mean = 50, sd = 0.5)  # custom mean and sd

set.seed(1303)             # fix random seed for reproducibility
x <- rnorm(20)
mean(x)
var(x)
```
...
**Build data frames from scratch and from random data:**

```{webr-r}
#| label: dataframe-basic
state     <- c("Alaska", "Arizona", "Arkansas")
...
set.seed(1303)
df <- data.frame(x = rnorm(20), y = rnorm(20, mean = 50, sd = 0.5))
head(df)
```
...
**Load `cars` (base R) and explore `Auto`:**
