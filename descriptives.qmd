...
* Why this matters with AI in the picture:
AI computes summaries. You decide which summary is honest, and you explain what it means.
...
### Central Tendency Lab

Load the `descriptives.csv` dataset as `descriptive_data`. It contains six variables: `ServiceScore`, `DeliveryTime`, `ReturnRate`, `LoyalCustomer`, `EngagementLevel`, and `CustomerValue`.
...
**1.** Compute the mean, median, and mode for `ServiceScore`.

::: {.callout-note collapse="true"}
### Show Answer

``` r
mean(descriptive_data$ServiceScore)
median(descriptive_data$ServiceScore)
sort(table(descriptive_data$ServiceScore), decreasing = TRUE)[1:5]
```

Mean: 50.00 \| Median: 50 \| Mode: 56 (appears most frequently). The mean and median are nearly identical, confirming a symmetric distribution.
:::

**2.** Compute the mean, median, and mode for `ReturnRate`.

::: {.callout-note collapse="true"}
### Show Answer

``` r
mean(descriptive_data$ReturnRate)
median(descriptive_data$ReturnRate)
sort(table(descriptive_data$ReturnRate), decreasing = TRUE)[1:5]
```

Mean: −0.06 \| Median: −0.06 \| All values appear once — no mode. The mean and median are almost identical, indicating a symmetric distribution centered near zero.
:::

**3.** For `ServiceScore`, the mean and median are nearly identical. What does that suggest about distribution shape?

::: {.callout-note collapse="true"}
### Show Answer

When the mean and median are equal or very close, the distribution is approximately symmetric — values are distributed evenly on both sides of the center. This is a sign that the mean is a reliable summary of the typical value and that no extreme outliers are distorting it.
:::
...
Continue using the `descriptives.csv` dataset. Load `semTools` if not already loaded.

``` r
library(semTools)
```

**1.** Run `skew()` and `kurtosis()` on `ServiceScore`. Record the z-values and interpret each.

::: {.callout-note collapse="true"}
### Show Answer

``` r
skew(data$ServiceScore)
kurtosis(data$ServiceScore)
```

ServiceScore has a z-score for skewness near 0 and kurtosis near 0 as well — both well within the ±3.29 threshold for a medium sample. ServiceScore is approximately symmetric and mesokurtic, making it suitable for analyses that assume normality.
:::

**2.** Run `skew()` and `kurtosis()` on `ReturnRate`. Record the z-values and interpret each.

::: {.callout-note collapse="true"}
### Show Answer

``` r
skew(data$ReturnRate)
kurtosis(data$ReturnRate)
```

ReturnRate has a z-score for skewness near 0 and a slightly negative kurtosis z-score — both within normal range. The distribution is approximately symmetric with slightly lighter tails than normal, consistent with the two low outliers identified in the boxplot lab.
:::

**3.** Which variable is suitable for analyses assuming normality? Which needs transformation first?

::: {.callout-note collapse="true"}
### Show Answer

Both `ServiceScore` and `ReturnRate` are approximately normal and ready to use without transformation. `DeliveryTime` is the variable in this dataset that requires transformation before applying parametric methods due to its extreme right skew and heavy tails.
:::

**4.** Calculate the standard deviation, variance, range, and IQR for `ServiceScore`.

::: {.callout-note collapse="true"}
### Show Answer

``` r
sd(data$ServiceScore)
var(data$ServiceScore)
range(data$ServiceScore)
IQR(data$ServiceScore)
```

SD ≈ 4.88 \| Variance ≈ 23.82 \| Range: 40 to 59 \| IQR ≈ 8. The tight range and small SD confirm that service scores cluster closely around the mean of 50.
:::

**5.** Calculate the same four statistics for `ReturnRate`.
...
sd(data$ReturnRate)
var(data$ReturnRate)
range(data$ReturnRate)
IQR(data$ReturnRate)
```

SD ≈ 1.00 \| Variance ≈ 1.00 \| Range: −3.24 to 2.50 \| IQR ≈ 1.33. The range is wider than the IQR suggests because the two extreme low values stretch the minimum downward, but the bulk of the data sits within a tight band near zero.
:::

**6.** `ServiceScore` has a much smaller standard deviation than `ReturnRate`. What does this say about consistency?

::: {.callout-note collapse="true"}
### Show Answer

The smaller SD for ServiceScore (≈4.88 on a 40–59 scale) means scores are very consistent — most customers rate service similarly. ReturnRate has an SD of ≈1.00 on a scale that spans about 5.75 units, meaning returns vary more meaningfully relative to the scale. Consistency in ServiceScore may reflect a standardized service process, while ReturnRate variability may reflect genuine differences in customer behavior.
:::
...
## Quantitative Variable Lab

Continue using the `descriptives.csv` dataset. Make sure `tidyverse` is loaded.

``` r
library(tidyverse)
```

**1.** Create a `geom_histogram()` of `ServiceScore` with a binwidth of 5, a non-black fill color, and an informative title.

::: {.callout-note collapse="true"}
### Show Answer

``` r
ggplot(data, aes(x = ServiceScore)) +
...
The histogram should appear roughly symmetric — scores range from 40 to 59 and are spread fairly evenly, with no obvious tail pulling in either direction.
:::

**2.** Create a histogram of `ReturnRate` with a binwidth of 0.5 and a different fill color.
...
ggplot(data, aes(x = ReturnRate)) +
  ...
  labs(title = "Distribution of Return Rate", x = "Return Rate", y = "Count")
```

The histogram should appear roughly bell-shaped and centered near zero. Two observations in the −3 to −3.5 range produce a slight leftward tail, but the bulk of the data is symmetric.
:::

**3.** Is each histogram symmetric or skewed? Does the visual match what the skewness statistics showed?

::: {.callout-note collapse="true"}
### Show Answer

Both `ServiceScore` and `ReturnRate` should appear roughly symmetric — consistent with the near-zero z-scores from the Spread Lab. The histograms confirm what the statistics reported. This is the intended workflow: run the numbers first, then use the visual to confirm.
:::

**4.** Create a histogram of `CustomerValue` with a binwidth of 15. Based on the visual, does the distribution appear symmetric, left-skewed, or right-skewed?

::: {.callout-note collapse="true"}
### Show Answer

``` r
ggplot(data, aes(x = CustomerValue)) +
  geom_histogram(binwidth = 15, fill = "steelblue", color = "white") +
  labs(title = "Distribution of Customer Value", x = "Customer Value", y = "Count")
```

The histogram should appear roughly symmetric with a slight right pull. The mean (96.62) and median (94) are close but not identical — consistent with z = 1.35 from the Spread Lab, which falls within the normal range.
:::

**5.** Create a boxplot of `ServiceScore` and a separate boxplot of `ReturnRate`. For each, identify whether any outliers appear as individual points beyond the whiskers.

::: {.callout-note collapse="true"}
### Show Answer

``` r
ggplot(data, aes(x = "", y = ServiceScore)) +
  geom_boxplot(fill = "mediumseagreen", color = "darkgreen") +
  labs(title = "Boxplot of Service Score", y = "Service Score", x = "")

ggplot(data, aes(x = "", y = ReturnRate)) +
  geom_boxplot(fill = "mediumpurple", color = "purple4") +
  labs(title = "Boxplot of Return Rate", y = "Return Rate", x = "")
```

`ServiceScore` has no outliers — the distribution is compact and symmetric. `ReturnRate` shows two outlier points at the lower end (values of approximately −3.24 and −3.19) plotted individually below the left whisker, consistent with the slight leftward tail visible in the histogram.
:::

**6.** Compute the IQR bounds for `ServiceScore` and identify any outlier values numerically. Then confirm with a boxplot.

::: {.callout-note collapse="true"}
### Show Answer

``` r
Q1      <- quantile(data$ServiceScore, 0.25)
Q3      <- quantile(data$ServiceScore, 0.75)
IQR_val <- IQR(data$ServiceScore)
lower_bound <- Q1 - 1.5 * IQR_val; lower_bound
upper_bound <- Q3 + 1.5 * IQR_val; upper_bound



descriptive_data[descriptive_data$ServiceScore < lower_bound | descriptive_data$ServiceScore > upper_bound, ]

ggplot(descriptive_data, aes(x = "", y = ServiceScore)) +
  geom_boxplot(fill = "mediumseagreen", color = "darkgreen") +
  labs(title = "Boxplot of Service Score", y = "Service Score", x = "")
```

Q1 = 47, Q3 = 55, IQR = 8. Lower bound = 35, upper bound = 67. All ServiceScore values fall between 40 and 59 — well within the bounds. No outliers are present, which the boxplot confirms: no individual points appear beyond the whiskers.
:::

**7.** Create density plots for `ServiceScore` and `ReturnRate`, each with a dashed vertical line at the mean. What does the position of the mean line relative to the peak tell you about each distribution?
...
ggplot(descriptive_data, aes(x = ServiceScore)) +
  geom_density(fill = "mediumseagreen", color = "darkgreen", alpha = 0.5) +
  geom_vline(aes(xintercept = mean(ServiceScore)), color = "red", linetype = "dashed") +
  labs(title = "Density of Service Score", x = "Service Score", y = "Density")

ggplot(descriptive_data, aes(x = ReturnRate)) +
  geom_density(fill = "mediumpurple", color = "purple4", alpha = 0.5) +
  geom_vline(aes(xintercept = mean(ReturnRate)), color = "red", linetype = "dashed") +
  labs(title = "Density of Return Rate", x = "Return Rate", y = "Density")
```

For `ServiceScore`, the mean (≈50.0) sits at the center of the peak — a symmetric distribution. For `ReturnRate`, the mean (≈−0.06) also falls near the center of the peak and close to zero, indicating a roughly symmetric spread centered just below zero. The two low outliers produce a slight leftward tail but do not meaningfully pull the mean away from the median.
:::
...
### Summarizing Qualitative Data Lab

Use the `descriptives.csv` dataset. The dataset contains two qualitative variables: `EngagementLevel` (high, medium, low) and `LoyalCustomer` (yes, no).



**1.** Calculate a frequency table, relative frequencies, cumulative frequencies, and cumulative proportions for `EngagementLevel`. Combine them into a transposed data frame.
...
**2.** Fill in the table from your output (levels appear in alphabetical order):
...
**3.** Which engagement level is most common? What proportion of customers fall into that category? Now run the same frequency analysis on `LoyalCustomer` — what percentage are loyal?

::: {.callout-note collapse="true"}
### Show Answer

Medium engagement is most common at 80 customers (40.0%). For `LoyalCustomer`:

``` r
prop.table(table(data$LoyalCustomer))
```

43.5% of customers are loyal (yes) and 56.5% are not (no).
:::
...
## Visualizing Qualitative Variables Lab

Continue using the `descriptives.csv` dataset. Make sure `tidyverse` is loaded.


**1.** Create a bar chart of `EngagementLevel` showing the count of customers in each category. Use `geom_bar()` with a fill color and an informative title.
...
`geom_bar()` counts observations automatically. The bars show medium (80) as tallest, followed by low (67) and high (53).
:::

**2.** Modify the chart from Question 1 to display **proportions** on the y-axis instead of counts.
...
`after_stat(prop)` calculates the proportion relative to the total. `group = 1` is required so ggplot computes proportions across the entire dataset. The y-axis shows medium at 40%, low at 33.5%, and high at 26.5%.
:::

**3.** Create a bar chart of `LoyalCustomer`. Add `geom_text()` to display the exact count above each bar.

::: {.callout-note collapse="true"}
### Show Answer

``` r
ggplot(descriptive_data, aes(x = LoyalCustomer)) +
  geom_bar(fill = "mediumseagreen", color = "white") +
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5, size = 4) +
  labs(title = "Loyal vs. Non-Loyal Customers",
       x = "Loyal Customer", y = "Count")
```

`stat = "count"` inside `geom_text()` matches the counting that `geom_bar()` performs and `after_stat(count)` pulls that count as the label. The chart shows 113 non-loyal (no) and 87 loyal (yes) customers.
:::

**4.** Create a **grouped** (side-by-side) bar chart showing `EngagementLevel` on the x-axis, with bars filled and grouped by `LoyalCustomer`. Use `position = "dodge"` and include a legend.
...
`position = "dodge"` places bars side by side. Within each engagement level, the non-loyal bar is consistently taller: high (27 no, 26 yes), low (39 no, 28 yes), medium (47 no, 33 yes). Loyal customers are a minority across all three engagement levels.
:::

**5.** Reorder the bars in Question 1 so they appear from most to least frequent. What function controls bar order in ggplot2?

::: {.callout-note collapse="true"}
### Show Answer

``` r
ggplot(descriptive_data, aes(x = reorder(EngagementLevel, 
                             -table(EngagementLevel)[EngagementLevel]))) +
  geom_bar(fill = "steelblue", color = "white") +
  labs(title = "Customer Count by Engagement Level (Ordered)",
       x = "Engagement Level", y = "Count")
```

`reorder()` controls bar order. Wrapping the x variable with `reorder(variable, -frequency)` sorts bars from tallest to shortest: medium (80) → low (67) → high (53). Ordering by frequency makes it immediately clear which category dominates.
:::
...
In this lesson, we explored descriptive statistics for both quantitative and qualitative data in R. For quantitative variables, we followed the natural sequence of analysis: we began with **central tendency** — calculating the mean, median, and mode using the `descriptives.csv` dataset and small vectors like salaries and GrowthFund. We then introduced the **histogram** as the visual tool for understanding distribution shape before moving into numerical measures of spread. Under **spread**, we covered variance and standard deviation for the mean, skewness and kurtosis to assess normality, and range and IQR paired with the median. The **boxplot** connects IQR to a visual five-number summary and makes outliers easy to spot. The **density plot** offers a smoothed alternative to the histogram, useful for comparing distributions or overlaying a mean reference line.

For qualitative variables, we calculated **frequencies, proportions, and cumulative distributions** using `table()`, `prop.table()`, and `cumsum()`, and organized results into a transposed data frame using `rbind()`, `t()`, and `as.data.frame()`. We also visualized categorical variables using **bar charts** with `geom_bar()` and **pie charts** with `coord_polar()`.

Each section includes a hands-on lab using the `descriptives.csv` dataset, which contains `CustomerValue`, `DeliveryTime`, `EngagementLevel`, `LoyalCustomer`, `ServiceScore`, and `ReturnRate`.

In the next lesson, we move into **data preparation** — data types, coercion, filtering, recoding, and reshaping using the `dplyr` package.
