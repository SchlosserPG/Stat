...
![Salary Data](Pictures/Ch2/Salaries.png "Salary Data")
...
mean(salaries)
```
* Note that due to at least one _outlier_ this mean does not reflect the typical salary - more on that later. 
...
* Variance is a measure of spread for numeric variables that is essentially the average of the squared differences between each observation value on some variable and the mean for that variable with population variance.
