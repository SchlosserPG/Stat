...
-   The same sampling limitations you are learning in this chapter apply directly to the AI tools you use every day. No AI training dataset is a perfectly representative sample of all human knowledge — every training corpus reflects biases in what text was collected, when it was collected, and whose perspectives it captured. This is why AI models have systematic blind spots and why hallucinations occur: when the correct answer is underrepresented in the training data, the model generates whatever is statistically most likely rather than whatever is true. The probabilistic reasoning you are learning here is foundational to understanding why.
...
## Chi-Squared Test of Independence — Imported Dataset
...
## Goodness-of-Fit Test — Imported Dataset

-   The steps are the same as the Chi-Squared Test of Independence.
-   Step 1: Set up the Hypothesis:
    -   $𝐻_0$: Each proportion equals a hypothesized value.
    -   $𝐻_A$: Not all proportions equal their hypothesized values.
-   Step 2: Compute the test statistic and calculate the probability.
...
Use the following prompts with our chatbot (bottom right of this page) to explore more about probability and contingency tables.

**Important verification note:** When using AI to calculate binomial or normal probabilities, always verify the result using R's `dbinom()`, `pbinom()`, `pnorm()`, or `qnorm()` functions directly. AI frequently gets multi-step probability calculations wrong — it is doing language prediction, not arithmetic. For any probability problem with more than one calculation step, treat AI as a starting point and run the code yourself to confirm the answer.
...
## Interactive R Lesson: Probability and Chi-Squared {#interactive-prob-lesson}

::: {.callout-note}
Parts 1-3 covers binomial and normal probability functions. Parts 4-6 covers contingency tables, probability rules, and the chi-squared test. All calculations use base R — no additional packages needed.
...
#| label: prob-setup
#| context: setup
# Base R only — no datasets to load
```

------------------------------------------------------------------------

### Part 1: Binomial Probabilities

The binomial distribution models the probability of a discrete yes/no outcome across a fixed number of trials. Two key functions:

-   `pbinom(x, n, p)` — cumulative probability: P(X ≤ x)
-   `dbinom(x, n, p)` — exact probability: P(X = x)

**P(X ≤ 5) given 10 trials, p = 0.5:**

```{webr-r}
pbinom(5, 10, .5)
```

**P(X \< 5) — "less than" means use x = 4:**
...
::: {.callout-important}
**Less than vs. less than or equal to** matters in discrete distributions. `P(X < 5)` = `P(X ≤ 4)`, so use 4 as the first argument. This distinction does **not** exist in the normal distribution because `P(X = x) = 0` for any exact value.
:::

**P(X \> 5) — flip the tail with `lower.tail = FALSE`:**

```{webr-r}
pbinom(5, 10, .5, lower.tail = FALSE)
```

**P(X ≥ 5) — greater than or equal to means use x = 4 with `lower.tail = FALSE`:**

```{webr-r}
pbinom(4, 10, .5, lower.tail = FALSE)
```

**P(X = 5) exactly — use `dbinom()`:**
...
::: {.callout-tip}
`dbinom()` has no `lower.tail` argument — it calculates the probability of exactly one value, so there is no tail to flip.
:::

------------------------------------------------------------------------

### Part 2: Normal Probabilities

The normal distribution is continuous, so `P(X = x) = 0` for any exact value. We always work with ranges using:

-   `pnorm(x, mean, sd)` — P(X \< x) or P(X ≤ x) — same result
-   `qnorm(p, mean, sd)` — the x value that has probability p below it

**P(X \< 3) with mean = 4, sd = 2:**
...
```{webr-r}
pnorm(3, 4, 2, lower.tail = FALSE)
```

::: {.callout-note}
For the normal distribution, `P(X < 3)` and `P(X ≤ 3)` are identical — and likewise `P(X > 3)` and `P(X ≥ 3)` are identical. This is because the probability of any exact value is zero in a continuous distribution.
:::

**`qnorm()` — find the x value associated with a given probability:**

```{webr-r}
# What value has 95% of the distribution below it?
qnorm(.95, 4, 2)

# What value has 5% of the distribution below it (= top 5%)?
qnorm(.05, 4, 2, lower.tail = FALSE)
```

::: {.callout-tip}
Both lines above produce the same answer — the 95th percentile from the left equals the 5th percentile from the right. They describe the same point on the distribution.
:::
...
**Apply the correct function to each real-world scenario.**

**Business survival — exactly 7 out of 20 survive (66% survival rate):**

```{webr-r}
# Discrete outcome (survive/not survive), asking for exactly 7 → dbinom
dbinom(7, 20, .66)
```

**Business survival — more than 10 out of 20 survive:**

```{webr-r}
# >10 means lower.tail = FALSE; no need to subtract 1
pbinom(10, 20, .66, lower.tail = FALSE)
```

**SAT scores — probability of scoring above 1400 (mean = 1250, sd = 300):**

```{webr-r}
# Continuous variable with mean and sd → pnorm; above → lower.tail = FALSE
pnorm(1400, 1250, 300, lower.tail = FALSE)
```
...
A contingency table organizes frequencies for two categorical variables. We build one from scratch using `matrix()`, then add row and column totals.
...
dimnames(M) <- list(c("Group A", "Group B"), c("Outcome 1", "Outcome 2"))
M

# Totals
total   <- sum(M)
rowsums <- margin.table(M, 1)   # 1 = rows
colsums <- margin.table(M, 2)   # 2 = columns
...
::: {.callout-tip}
`margin.table(M, 1)` sums across rows (giving row totals). `margin.table(M, 2)` sums across columns (giving column totals). The `1` and `2` refer to the margin dimension.
:::

**Combine into a complete frequency table using `cbind()` and `rbind()`:**

```{webr-r}
FrequencyTable <- cbind(M, rowsums)
colsums        <- c(colsums, total)
FrequencyTable <- rbind(FrequencyTable, colsums)
FrequencyTable
```

------------------------------------------------------------------------

### Part 5: Probability Table and Probability Rules
...
ProbabilityTable <- cbind(prob, rowprob)
colprob          <- c(colprob, totalProb)
ProbabilityTable <- rbind(ProbabilityTable, colprob)
ProbabilityTable
```

::: {.callout-note}
The center values (.20, .30, .10, .40) are **joint probabilities** — the probability of two events occurring together (the intersection). The margin values (.50, .50, .30, .70) are **marginal probabilities** — the unconditional probability of one event regardless of the other.
:::

**Addition rule — P(Group A or Outcome 1):**
...
**Complement rule — P(not Group B):**

```{webr-r}
1 - .5
```

**Conditional probability — P(Outcome 1 \| Group A already occurred):**

```{webr-r}
# P(Outcome 1 | Group A) = P(intersection) / P(Group A)
.2 / .5
```

------------------------------------------------------------------------

### Part 6: Chi-Squared Test of Independence

The chi-squared test of independence asks: are two categorical variables associated, or are they independent?

-   **H₀:** Group and Outcome are independent
-   **Hₐ:** Group and Outcome are dependent (associated)
...
::: {.callout-note}
The p-value of approximately 0.05 is at the boundary of the conventional alpha = 0.05 threshold. A p-value below 0.05 leads to rejecting H₀ and concluding dependence. Always report the exact p-value rather than just "reject" or "fail to reject."
:::

------------------------------------------------------------------------

## Scored Quiz: Probability and Chi-Squared {#prob-quiz}

```{=html}
<div id="ps512-prob-quiz" style="font-family: Georgia, serif; max-width: 720px; margin: 2rem auto; background: #fafaf8; border: 1px solid #ddd; border-radius: 8px; padding: 2rem;">
  ...
  <h3 style="margin-top:0; font-size:1.2rem; background:#2c6fad; color:#ffffff; padding:0.8rem 1.2rem; border-radius:6px 6px 0 0; margin:-2rem -2rem 1.5rem -2rem;">
    Probability and Chi-Squared Quiz
  </h3>

  <div id="pquiz-progress" style="font-size:0.85rem; color:#444; margin-bottom:1.5rem;">Question 1 of 10</div>

  <!-- Q1 -->
  <div class="pquiz-q" id="pq1" style="display:block">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">1. To calculate P(X ≤ 5) for a binomial distribution with n = 10 and p = 0.5, you would use:</p>
    <label class="pquiz-option"><input type="radio" name="pq1" value="a"> <code>dbinom(5, 10, 0.5)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq1" value="b"> <code>pbinom(5, 10, 0.5)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq1" value="c"> <code>pnorm(5, 10, 0.5)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq1" value="d"> <code>pbinom(6, 10, 0.5)</code></label>
    <div class="pquiz-feedback" id="pfb1"></div>
  </div>

  <!-- Q2 -->
  <div class="pquiz-q" id="pq2" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">2. For a binomial distribution, P(X &lt; 5) with n = 10, p = 0.5 is calculated as:</p>
    <label class="pquiz-option"><input type="radio" name="pq2" value="a"> <code>pbinom(5, 10, 0.5)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq2" value="b"> <code>pbinom(4, 10, 0.5)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq2" value="c"> <code>pbinom(5, 10, 0.5, lower.tail = FALSE)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq2" value="d"> <code>dbinom(5, 10, 0.5)</code></label>
    <div class="pquiz-feedback" id="pfb2"></div>
  </div>

  <!-- Q3 -->
  <div class="pquiz-q" id="pq3" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">3. Why does P(X &lt; 3) equal P(X ≤ 3) in a normal distribution but not in a binomial distribution?</p>
    <label class="pquiz-option"><input type="radio" name="pq3" value="a"> Because the normal distribution has a larger sample size.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq3" value="b"> Because P(X = 3) = 0 in a continuous distribution, so adding or excluding it changes nothing.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq3" value="c"> Because the binomial distribution uses lower.tail = FALSE.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq3" value="d"> Because pnorm() and pbinom() use different formulas.</label>
    <div class="pquiz-feedback" id="pfb3"></div>
  </div>

  <!-- Q4 -->
  <div class="pquiz-q" id="pq4" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">4. To find the probability that x &gt; 3 given a normal distribution with mean = 4 and sd = 2, you would use:</p>
    <label class="pquiz-option"><input type="radio" name="pq4" value="a"> <code>pnorm(3, 4, 2)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq4" value="b"> <code>qnorm(3, 4, 2)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq4" value="c"> <code>pnorm(3, 4, 2, lower.tail = FALSE)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq4" value="d"> <code>dbinom(3, 4, 2)</code></label>
    <div class="pquiz-feedback" id="pfb4"></div>
  </div>

  <!-- Q5 -->
  <div class="pquiz-q" id="pq5" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">5. <code>qnorm(0.95, 4, 2)</code> and <code>qnorm(0.05, 4, 2, lower.tail = FALSE)</code> produce:</p>
    <label class="pquiz-option"><input type="radio" name="pq5" value="a"> Different values — one is the 95th percentile, the other is the 5th.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq5" value="b"> The same value — both identify the same point on the distribution.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq5" value="c"> Different values — lower.tail changes the mean of the distribution.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq5" value="d"> An error — qnorm() does not accept lower.tail.</label>
    <div class="pquiz-feedback" id="pfb5"></div>
  </div>

  <!-- Q6 -->
  <div class="pquiz-q" id="pq6" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">6. 66% of startups survive their first two years. In a sample of 20, you want the probability that <em>exactly</em> 7 survive. You would use:</p>
    <label class="pquiz-option"><input type="radio" name="pq6" value="a"> <code>pbinom(7, 20, 0.66)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq6" value="b"> <code>pnorm(7, 20, 0.66)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq6" value="c"> <code>dbinom(7, 20, 0.66)</code></label><br>
    <label class="pquiz-option"><input type="radio" name="pq6" value="d"> <code>qnorm(7, 20, 0.66)</code></label>
    <div class="pquiz-feedback" id="pfb6"></div>
  </div>

  <!-- Q7 -->
  <div class="pquiz-q" id="pq7" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">7. In the contingency table built in the lesson, what do the center values (.20, .30, .10, .40) represent?</p>
    <label class="pquiz-option"><input type="radio" name="pq7" value="a"> Marginal probabilities — the probability of each group or outcome individually.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq7" value="b"> Joint probabilities — the probability of two events occurring together.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq7" value="c"> Conditional probabilities — the probability of one event given another.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq7" value="d"> Expected counts under the null hypothesis.</label>
    <div class="pquiz-feedback" id="pfb7"></div>
  </div>

  <!-- Q8 -->
  <div class="pquiz-q" id="pq8" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">8. The addition rule for P(Group A or Outcome 1) is:</p>
    <label class="pquiz-option"><input type="radio" name="pq8" value="a"> P(Group A) × P(Outcome 1)</label><br>
    <label class="pquiz-option"><input type="radio" name="pq8" value="b"> P(Group A) + P(Outcome 1) − P(Group A and Outcome 1)</label><br>
    <label class="pquiz-option"><input type="radio" name="pq8" value="c"> P(Group A) + P(Outcome 1)</label><br>
    <label class="pquiz-option"><input type="radio" name="pq8" value="d"> 1 − P(Group A) − P(Outcome 1)</label>
    <div class="pquiz-feedback" id="pfb8"></div>
  </div>

  <!-- Q9 -->
  <div class="pquiz-q" id="pq9" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">9. The conditional probability P(Outcome 1 | Group A) is calculated as:</p>
    <label class="pquiz-option"><input type="radio" name="pq9" value="a"> P(Outcome 1) + P(Group A)</label><br>
    <label class="pquiz-option"><input type="radio" name="pq9" value="b"> P(Group A) / P(Outcome 1)</label><br>
    <label class="pquiz-option"><input type="radio" name="pq9" value="c"> P(Outcome 1 ∩ Group A) / P(Group A)</label><br>
    <label class="pquiz-option"><input type="radio" name="pq9" value="d"> P(Outcome 1) × P(Group A)</label>
    <div class="pquiz-feedback" id="pfb9"></div>
  </div>

  <!-- Q10 -->
  <div class="pquiz-q" id="pq10" style="display:none">
    <p style="font-weight:600; color:#1a1a1a; margin-bottom:1rem;">10. In a chi-squared test of independence, the null hypothesis states:</p>
    <label class="pquiz-option"><input type="radio" name="pq10" value="a"> The two variables are dependent — there is a significant association.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq10" value="b"> The two variables are independent — there is no association.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq10" value="c"> The observed counts equal the expected counts exactly.</label><br>
    <label class="pquiz-option"><input type="radio" name="pq10" value="d"> The p-value is less than 0.05.</label>
    <div class="pquiz-feedback" id="pfb10"></div>
  </div>

  <!-- Navigation -->
  <div style="display:flex; gap:1rem; margin-top:1.5rem; align-items:center; flex-wrap:wrap;">
    <button id="pquiz-prev" onclick="pquizNav(-1)" style="display:none; padding:0.5rem 1.2rem; background:#6c757d; color:white; border:none; border-radius:5px; cursor:pointer; font-size:0.95rem;">← Previous</button>
    <button id="pquiz-next" onclick="pquizNav(1)" style="padding:0.5rem 1.2rem; background:#4a90d9; color:white; border:none; border-radius:5px; cursor:pointer; font-size:0.95rem;">Next →</button>
    <button id="pquiz-submit" onclick="psubmitQuiz()" style="display:none; padding:0.5rem 1.4rem; background:#27ae60; color:white; border:none; border-radius:5px; cursor:pointer; font-size:0.95rem; font-weight:600;">Submit Quiz</button>
  </div>

  <div id="pquiz-score" style="display:none; margin-top:1.5rem; padding:1rem 1.5rem; border-radius:6px; font-size:1rem;"></div>
</div>

<style>
.pquiz-option { display:block; margin:0.4rem 0; cursor:pointer; padding:0.4rem 0.6rem; border-radius:4px; transition:background 0.15s; color:#1a1a1a; }
.pquiz-option:hover { background:#ddeeff; border:1px solid #2c6fad; }
.pquiz-feedback { margin-top:0.8rem; padding:0.6rem 1rem; border-radius:5px; font-size:0.9rem; display:none; }
.pcorrect-fb { background:#d4edda; color:#155724; border-left:4px solid #27ae60; }
.pincorrect-fb { background:#f8d7da; color:#721c24; border-left:4px solid #e74c3c; }
</style>

<script>
const pAnswers = {
  pq1:'b', pq2:'b', pq3:'b', pq4:'c', pq5:'b',
  pq6:'c', pq7:'b', pq8:'b', pq9:'c', pq10:'b'
};
const pHints = {
  pq1:  "pbinom() calculates cumulative probability (≤). dbinom() is for exact values only.",
  pq2:  "P(X < 5) = P(X ≤ 4) in a discrete distribution. Use 4 as the first argument — not 5.",
  pq3:  "In a continuous distribution, the probability of any single exact value is zero. Adding or excluding it from a cumulative probability has no effect.",
  pq4:  "lower.tail = FALSE flips the calculation to give the area above x rather than below it.",
  pq5:  "The 95th percentile from the left is the same point as the 5th percentile from the right — they describe the same location on the distribution.",
  pq6:  "dbinom() is for exact probabilities in a discrete (binomial) setting. 'Exactly 7' means dbinom, not pbinom.",
  pq7:  "Joint probabilities are in the interior of the table — they represent P(Group A AND Outcome 1), i.e. the intersection. Marginal probabilities are in the margins.",
  pq8:  "The addition rule subtracts the intersection to avoid double-counting: P(A or B) = P(A) + P(B) − P(A and B).",
  pq9:  "Conditional probability divides the joint probability by the probability of the given condition: P(B|A) = P(A∩B) / P(A).",
  pq10: "In a chi-squared test of independence, H₀ always states independence. Dependence (association) is always in the alternative hypothesis."
};
let pcurrent = 1;
const ptotal = 10;

function pquizNav(dir) {
  document.getElementById('pq' + pcurrent).style.display = 'none';
  pcurrent += dir;
  document.getElementById('pq' + pcurrent).style.display = 'block';
  document.getElementById('pquiz-progress').textContent = 'Question ' + pcurrent + ' of ' + ptotal;
  document.getElementById('pquiz-prev').style.display = pcurrent > 1 ? 'inline-block' : 'none';
  document.getElementById('pquiz-next').style.display = pcurrent < ptotal ? 'inline-block' : 'none';
  document.getElementById('pquiz-submit').style.display = pcurrent === ptotal ? 'inline-block' : 'none';
}

function psubmitQuiz() {
  let score = 0;
  for (let i = 1; i <= ptotal; i++) {
    const sel = document.querySelector('input[name="pq' + i + '"]:checked');
    const fb  = document.getElementById('pfb' + i);
    document.getElementById('pq' + i).style.display = 'block';
    if (sel && sel.value === pAnswers['pq' + i]) {
      score++;
      fb.className = 'pquiz-feedback pcorrect-fb';
      fb.textContent = '✓ Correct! ' + pHints['pq' + i];
    } else {
      fb.className = 'pquiz-feedback pincorrect-fb';
      fb.textContent = '✗ ' + (sel ? 'Not quite. ' : 'No answer selected. ') + pHints['pq' + i];
    }
    fb.style.display = 'block';
  }
  document.getElementById('pquiz-prev').style.display = 'none';
  document.getElementById('pquiz-next').style.display = 'none';
  document.getElementById('pquiz-submit').style.display = 'none';
  document.getElementById('pquiz-progress').textContent = 'Quiz complete';
  const sd = document.getElementById('pquiz-score');
  const pct = Math.round((score / ptotal) * 100);
  ...
  sd.innerHTML = '<strong>Your Score: ' + score + ' / ' + ptotal + ' (' + pct + '%)</strong><br>' +
