...
**Cumulative probabilities with `pbinom()` — all four inequality cases:**

```{webr-r}
#| label: pbinom-all
pbinom(5, 10, .5)                      # P(X ≤ 5)
pbinom(4, 10, .5)                      # P(X < 5) — use x=4 for strict less than
pbinom(5, 10, .5, lower.tail = FALSE)  # P(X > 5)
pbinom(4, 10, .5, lower.tail = FALSE)  # P(X ≥ 5) — use x=4 with upper tail
```
...
**Exact probability with `dbinom()`:**

```{webr-r}
#| label: dbinom-exact
dbinom(5, 10, .5)  # P(X = 5) exactly — no lower.tail argument
```
...
**Normal probabilities with `pnorm()` and `qnorm()`:**

```{webr-r}
#| label: pnorm-all
pnorm(3, 4, 2)                      # P(X < 3) — same as P(X ≤ 3)
pnorm(3, 4, 2, lower.tail = FALSE)  # P(X > 3)
qnorm(.95, 4, 2)                    # x value with 95% below it
qnorm(.05, 4, 2, lower.tail = FALSE) # same point — top 5%
```
...
```{webr-r}
#| label: word-problems
# Exactly 7 of 20 survive (66% rate) — discrete exact → dbinom
dbinom(7, 20, .66)

# More than 10 of 20 survive — cumulative upper tail → pbinom lower.tail=FALSE
pbinom(10, 20, .66, lower.tail = FALSE)

# SAT score above 1400 (mean=1250, sd=300) — continuous upper tail → pnorm
...
**Probability rules — addition, complement, and conditional:**

```{webr-r}
#| label: prob-rules
# Addition rule: P(Group A or Outcome 1) = P(A) + P(B) - P(A and B)
.5 + .3 - .2

# Complement rule: P(not Group B)
1 - .5

# Conditional probability: P(Outcome 1 | Group A) = intersection / P(Group A)
.2 / .5
```
